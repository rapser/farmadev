...
struct CouponDetailDTO: Codable {
    let companyName: String?
    let data: DataDetailCoupDTO?
    let error: String?
    
    // Modelos auxiliares incluidos directamente
    struct DataDetailCoupDTO: Codable {
        let id: String?
        let sku: String?
        ...
        let categoryRelation: [CategoryRelationDTO]?
        let pathImage: PathImageDetailDTO?
    ...
    struct CategoryRelationDTO: Codable {
        let categoryCode: Int?
        let priority: Int?
    }
    
    struct PathImageDetailDTO: Codable {
        let small: String?
        let medium: String?
        let large: String?
    }
